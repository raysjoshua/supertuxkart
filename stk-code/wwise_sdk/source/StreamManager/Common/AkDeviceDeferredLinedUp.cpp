	...
	)
{
	...
	if ( AK_Success == eResult )
	{
		...
		m_pLowLevelTransfersMem = (CAkLowLevelTransferDeferred*)AkAlloc( AkMemID_Streaming, in_settings.uMaxConcurrentIO * sizeof( CAkLowLevelTransferDeferred ) );
...
{
	// Validation: if num low-level xfers is 0 then GetNumConcurrentIO() must be 0 - not waiting for unordered request completion.
    ...
    )
{
	...
	if ( !pNewStm )
	{
		...
		pNewStm = AkNew( AkMemID_Streaming, CAkStdStmDeferredLinedUp() );
	}
    ...
    )
{
	...
	if ( !pNewStm )
	{
		...
		pNewStm = AkNew( AkMemID_Streaming, CAkAutoStmDeferredLinedUp() );
	}
...
{
	AkUInt32 uMaxQueueableTasks = m_uMaxConcurrentIO - GetNumConcurrentIO();
	AkUInt32 uTotalTasksQueued = 0;

	// Determine list of tasks to be executed
	// Note that the first item in each array below is for Read transfers, the second is for Write transfers
	AkUInt32 uNumTransfersQueued[2] = { 0, 0 };
	IAkIOHookDeferredBatch::BatchIoTransferItem* pBatchTransferInfo[2] =
	{
		(IAkIOHookDeferredBatch::BatchIoTransferItem*)AkAlloca(sizeof(IAkIOHookDeferredBatch::BatchIoTransferItem)*uMaxQueueableTasks),
		(IAkIOHookDeferredBatch::BatchIoTransferItem*)AkAlloca(sizeof(IAkIOHookDeferredBatch::BatchIoTransferItem)*uMaxQueueableTasks)
	};

	{
		...
		while (uTotalTasksQueued < uMaxQueueableTasks && (pTask = SchedulerFindNextTask(fOpDeadline)))
		{
			...
			AKRESULT eResult = pTask->EnsureFileIsOpen();
			if (eResult != AK_Success)
			{
				// Deferred open failed. Update/Kill this task and skip it.
				pTask->Kill();
				continue;
			}
			...
			if (!pMemView)
			{
				...
				// Update as "canceled" and skip this task.
			...
			if (!pLowLevelXferDeferred->GetWasSentToLLIO())
			{
				...
				AkUInt32 uInfoArray = pTask->IsWriteOp() ? 1 : 0;
				AkUInt32 uIdx = uIdx = uNumTransfersQueued[uInfoArray];
	...
	{
		IAkIOHookDeferredBatch* pIoHookDeferred = static_cast<IAkIOHookDeferredBatch*>(m_pLowLevelHook);
		...
		for (AkUInt32 uOpIdx = 0; uOpIdx < 2; ++uOpIdx)
		{
			...
			AKRESULT cumulativeResult;
			AKRESULT* pDispatchResults = (AKRESULT*)AkAlloca(sizeof(AKRESULT) * uNumTransfers);
			IAkIOHookDeferredBatch::BatchIoTransferItem* pTransferInfoList = pBatchTransferInfo[uOpIdx];
			if (uOpIdx == 0)
			{
				cumulativeResult = pIoHookDeferred->BatchRead(
					uNumTransfers,
					pTransferInfoList,
					CAkLowLevelTransferDeferred::LLBatchIOCallback,
					pDispatchResults);
			}
			else
			{
				cumulativeResult = pIoHookDeferred->BatchWrite(
					uNumTransfers,
					pTransferInfoList,
					CAkLowLevelTransferDeferred::LLBatchIOCallback,
					pDispatchResults);
			...
			// Fire batch callback for the tasks that were not dispatched successfully
			if (cumulativeResult != AK_Success)
			{
				AkUInt32 uNumFailedDispatches = 0;
				AKRESULT* pFailedDispatchResults = (AKRESULT*)AkAlloca(sizeof(AKRESULT) * uNumTransfers);
				AkAsyncIOTransferInfo** ppFailedDispatches = (AkAsyncIOTransferInfo**)AkAlloca(sizeof(AkAsyncIOTransferInfo*) * uNumTransfers);

				for (AkUInt32 i = 0; i < uNumTransfers; ++i)
				{
					if (pDispatchResults[i] != AK_Success)
					{
						ppFailedDispatches[uNumFailedDispatches] = pTransferInfoList[i].pTransferInfo;
						pFailedDispatchResults[uNumFailedDispatches] = pDispatchResults[i];
						uNumFailedDispatches++;
					}
				}
				if (uNumFailedDispatches > 0)
				{
					CAkLowLevelTransferDeferred::LLBatchIOCallback(uNumFailedDispatches, ppFailedDispatches, pFailedDispatchResults);
				}
			}
		}
	}
	...
	// Cancel pending transfers if applicable.
	{
...

		{
			...
			while ( it != m_listCancelledXfers.End() )
			{
				...
				pTransfer->Cancel( 
					static_cast<IAkIOHookDeferredBatch*>( m_pDevice->GetLowLevelHook() ),
					bCallLowLevelIO, 
					bAllCancelled );
