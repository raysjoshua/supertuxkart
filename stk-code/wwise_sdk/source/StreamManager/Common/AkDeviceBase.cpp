...
CAkStmTask::~CAkStmTask()
{
	...
	if ( m_pFileDesc )
		AkFree( AkMemID_Streaming, m_pFileDesc );
}
...
AKRESULT  CAkAutoStmBase::SetMinTargetBufferSize(
	AkUInt32		in_uMinTargetBufferSize	///< Minimum size to buffer ahead in an automated stream.
)
{
	m_uMinimalBufferingSize = in_uMinTargetBufferSize;
	return AK_Success;
}
...
AKRESULT AK::StreamMgr::CAkAutoStmMemory::SetMinTargetBufferSize(AkUInt32 in_uMinTargetBufferSize)
{
	m_uMinTargetBufferSize = in_uMinTargetBufferSize;
	return AK_Success;
}
...
AKRESULT AK::StreamMgr::CAkAutoStmMemory::SetPosition(AkInt64 in_iMoveOffset, AkMoveMethod in_eMoveMethod, AkInt64 * out_piRealOffset)
{
	AkInt64 iPosition;
	if ( in_eMoveMethod == AK_MoveBegin )
	{
	...
	if (out_piRealOffset != NULL)
	{
		*out_piRealOffset = in_iMoveOffset;
	}
