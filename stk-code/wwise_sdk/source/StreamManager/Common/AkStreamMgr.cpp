...
{
	AkDeferredOpenData * pDeferredOpenData = (AkDeferredOpenData*)AkAlloc( AkMemID_Streaming, sizeof( AkDeferredOpenData ) );
...
void AkDeferredOpenData::Term()
{
	if ( bByString 
		&& pszFileName )
	{
		AkFree( AkMemID_Streaming, pszFileName );
	}
}
	...
	)
{
	...
	if ( in_settings.uSchedulerTypeFlags & AK_SCHEDULER_BLOCKING )
	{
		...
		pNewDevice = AkNew( AkMemID_Streaming, CAkDeviceBlocking( in_pLowLevelHook ) );
		if ( pNewDevice != NULL )
		{
	...
	)
{
	...
	AkFileDesc * pFileDesc = (AkFileDesc*)AkAlloc( AkMemID_Streaming, sizeof( AkFileDesc ) );
	if ( !pFileDesc )
		return AK_Fail;
	...
	{
		AkFree( AkMemID_Streaming, pFileDesc );
