...
namespace StreamMgr
{
	...
	public:
		...
		static void LLBatchIOCallback(
			AkUInt32 in_uNumTransfers,
			AkAsyncIOTransferInfo** in_ppTransferInfo, //< Pointer to the AkAsyncIOTransferInfo structure that was passed to corresponding Read() or Write() call.
			AKRESULT* in_peResult //< Result of transfer;
		);
	...
	private:
		...
		AkUInt32		m_bWasLLIOCancelCalled	:1;	// This bit is set when IAkIOHookDeferredBatch::Cancel() is called (to avoid calling it more than once).
