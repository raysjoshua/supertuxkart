...
AkMemBlock* CAkIOMemMgr::AllocMemBlock( AkUInt32 in_uAllocSize, AkUInt32 in_uRequestedSize, AkUInt32 in_uAlignment )
{
	...
	if (pIOMemory)
	{
		...
		AkMemBlock* pBlk = AkNew( AkMemID_Streaming, AkMemBlock( pIOMemory ) );
		if (pBlk)
		{
...
void CAkIOMemMgr::FreeMemBlock( AkMemBlock* in_pBlk )
{
	...
	AkDelete(AkMemID_Streaming, in_pBlk );
	...
	)
{
	...
	out_pMemBlock = (AkMemBlock*)AkAlloc( AkMemID_Streaming, sizeof( AkMemBlock ) );
	if ( !out_pMemBlock )
		return;
	...
	)
{
	...
	{
		AkFree( AkMemID_Streaming, in_pMemBlock );
	}
}
