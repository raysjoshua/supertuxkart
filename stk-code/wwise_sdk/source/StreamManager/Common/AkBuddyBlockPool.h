...
public:
	...
	enum{ kUserPool = 0xFFFFFFFF };

	AkBuddyBlockPool() : m_pMem(NULL), m_uPoolSize(0), m_uMaxBlockSize(0), m_uMinBlockSize(0), m_uShiftForMinBlockSize(0), m_uNumLevels(0), m_uPoolAlloc(kUserPool)
	{
	}

	AKRESULT Init(AkUInt32 in_uPoolSize, AkUInt32 in_uMaxBlockSize, AkUInt32 in_uMinBlockSizeAndAlign, AkUInt32 in_eAttributes, void* in_pUserAllocatedMem = NULL)
	{
		...
		if (m_uPoolSize > 0)
		{
			if (in_pUserAllocatedMem)
			{
				m_pMem = in_pUserAllocatedMem;
			}
			else
			{
				m_uPoolAlloc = AkMemID_StreamingIO | in_eAttributes;
				m_pMem = AkMalign(m_uPoolAlloc, in_uPoolSize, in_uMinBlockSizeAndAlign);
			}
	...
	{
		CHECK_POOL();
		if (m_pMem && m_uPoolAlloc != kUserPool)
		{
			AkFree(m_uPoolAlloc, m_pMem);
			m_pMem = NULL;
		}
	}
...
private:
	...
	AkUInt32 m_uPoolAlloc; // kUserPool in case of user allocation
...
public:
	...
	AK::MemoryMgr::CategoryStats m_Stats;
