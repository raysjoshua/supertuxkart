...
namespace AK
{
	namespace DSP
	{
			...
			AkReal32 in_fTargetGain /* = 1.f */)
		{
			...
			const AKSIMD_V4F32 vPI = AKSIMD_SET_V4F32( fPI );
			const AKSIMD_V4F32 vTWOPI = AKSIMD_SET_V4F32( fTWOPI );
			...
			AK_FPSetValGTE( fInPhase[1], fPI, fInPhase[1], fInPhase[1] - fTWOPI );
			fInPhase[2] = fInPhase[1] + fPhaseIncrement;
			AK_FPSetValGTE( fInPhase[2], fPI, fInPhase[2], fInPhase[2] - fTWOPI );
			fInPhase[3] = fInPhase[2] + fPhaseIncrement;
			AK_FPSetValGTE( fInPhase[3], fPI, fInPhase[3], fInPhase[3] - fTWOPI );
			fPhaseIncrement *= 4.f;
			// Support for frequency higher than Sr/4
			AK_FPSetValGTE( fPhaseIncrement, fTWOPI, fPhaseIncrement, fPhaseIncrement - fTWOPI );
			...
			// Phase wrap constants
			AKSIMD_BUILD_V2F32( const AKSIMD_V2F32 vPI, fPI, fPI );
			AKSIMD_BUILD_V2F32( const AKSIMD_V2F32 vTWOPI, fTWOPI, fTWOPI );

			AkReal32 fPhaseIncrement = m_fPhaseIncrement;
			AkReal32 fInPhase0 = m_fPhaseOffset;
			AkReal32 fInPhase1 = fInPhase0 + fPhaseIncrement;
			AK_FPSetValGTE( fInPhase1, fPI, fInPhase1, fInPhase1 - fTWOPI );
			fPhaseIncrement *= 2.f;
			// Support for frequency higher than Sr/4
			AK_FPSetValGTE( fPhaseIncrement, fTWOPI, fPhaseIncrement, fPhaseIncrement - fTWOPI );
