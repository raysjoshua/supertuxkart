...
void PerforceSourceControl::Add(const CStringList& in_rOriginalFilenameList)
{
	...
	POSITION rPosition = files.GetHeadPosition();
	...
	if (files.GetCount() > 0)
	{
		// Convert filenames
		CStringList escapedFilenameList;
		GetValidPathList(&files, &escapedFilenameList, false);

		// Get the status of the files
		m_fileStatusMap.RemoveAll();
		OperationResult m_operationResult = OperationResult_Succeed;
		m_operationResult = GetFileStatus(in_rOriginalFilenameList, NULL, true);
		...
		for (int i = 0; i < files.GetCount(); i++) {
			...
			CString csFilename = files.GetNext(rPosition);
		...
		if (bReconcile) 
		{
			...
			RunCommand("reconcile", &files, NULL); // reconcile will open file for edit instead of letting it marked for delete
