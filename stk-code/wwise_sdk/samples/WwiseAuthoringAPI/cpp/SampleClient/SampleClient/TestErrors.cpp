...
static void ErrorLogger(const char* logLine)
{
...
void TestErrors()
{
...

	{
		...
		Logger::Get()->SetLoggerFunction(ErrorLogger);
		...
		// Calling with the wrong URI.
		{
			...
			{
				std::string resultJsonString = RapidJsonUtils::GetAkJsonString(resultJson);
				auto message = resultJson["message"].GetVariant().GetString();
				std::cout << resultJsonString << std::endl;
			}
			else
			{
				std::cout << "Expected an error." << std::endl;
