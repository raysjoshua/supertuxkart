...
DemoOptions* DemoOptions::s_pThisDemoPage;
...
{
	s_pThisDemoPage = this;
	AK::Monitor::SetLocalOutput(AK::Monitor::ErrorLevel_Error, LocalErrorCallback);
...
{	
	...
	s_pThisDemoPage = NULL;
	Page::Release();
}

//Use the LocalOutputFunc to display errors in string, from the sound engine.
void DemoOptions::LocalErrorCallback(
...
{
	char* pMsg;
	CONVERT_OSCHAR_TO_CHAR(in_pszError, pMsg);
	if (s_pThisDemoPage)
		s_pThisDemoPage->m_InternalErrorMsg = pMsg;
}
...
void DemoOptions::InitSpeakerPanning()
{
	...
	AkPanningRule rule;
	AK::SoundEngine::GetPanningRule(rule);	//Get the panning rule for the main device.
	newToggle->SetSelectedIndex((int)rule);
