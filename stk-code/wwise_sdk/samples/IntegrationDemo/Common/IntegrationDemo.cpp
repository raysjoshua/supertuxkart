...
#include "Wwise_IDs.h"		// IDs generated by Wwise
	...
	#include "AkFilePackageLowLevelIODeferred.h"		// Low level io (deferred)
#else
...
m_bGamePaused(false)
{
	...
	m_pMenu = new Menu();
    m_pLowLevelIO = new LowLevelIOSystem();
...
LowLevelIOSystem& IntegrationDemo::IOManager()
{
...
#if defined(INTDEMO_DEFERRED_IO)
	m_deviceSettings.uSchedulerTypeFlags = AK_SCHEDULER_DEFERRED_LINED_UP;
#endif
...
const std::vector<const char*> IntegrationDemo::GetSupportedAudioDeviceSharesets()
{
	...
	if (supportedSharesets.empty())
	{
		...
		// This would usually be a secondary output device but in this demo
		...
		supportedSharesets.push_back("Communication_Output");
#elif defined(AK_XBOX)
		// XboxOne supports the following sharesets but can use and will list the same devices for all of them.
