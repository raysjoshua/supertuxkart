...
namespace d3d12
{
...
#else
		static RenderDeviceXboxOne instance;
#endif
		return instance;
	}
	...
	bool RenderDeviceXboxOne::Init(void* data)
	{
		m_window = reinterpret_cast<IUnknown*>(data);
		return RenderDevice::Init(data);
	}

	void RenderDeviceXboxOne::Term()
	{
		m_swapChain.Reset();
		return RenderDevice::Term();
	}

	ComPtr<ID3D12Device> RenderDeviceXboxOne::CreateDevice()
	{
		ComPtr<ID3D12Device2> device;
	...
	void RenderDeviceXboxOne::CreateSwapChain()
	{
		// First, retrieve the underlying DXGI device from the D3D device.
		ComPtr<IDXGIDevice1> dxgiDevice;
		ThrowIfFailed(m_device.As(&dxgiDevice));
		...
		ThrowIfFailed(dxgiFactory->CreateSwapChainForCoreWindow(
			m_device.Get(), // Xbox One uses device here, not the command queue
			m_window,
			&swapChainDesc,
			nullptr,
	...
	void RenderDeviceXboxOne::UpdateRenderTargetViews()
	{
		CD3DX12_CPU_DESCRIPTOR_HANDLE rtvHandle(m_RTVDescriptorHeap->GetCPUDescriptorHandleForHeapStart());

		for (size_t i = 0; i < m_backBuffers.size(); ++i)
		{
			ThrowIfFailed(m_swapChain->GetBuffer((UINT)i, PLATFORM_IDD_PPV_ARGS(m_backBuffers[i].GetAddressOf())));
	...
	void RenderDeviceXboxOne::Present()
	{
		ThrowIfFailed(m_swapChain->Present(0, 0));
	}
#endif
