...
void ToggleControl::Draw( DrawStyle in_eDrawStyle )
{
	...
	if ( m_Options.size() )
	{
		if ( (in_eDrawStyle == DrawStyle_Selected || in_eDrawStyle == DrawStyle_SelectedText) && m_Options.size() > 1 )
		{
...
void ToggleControl::SetSelectedIndex(int val)
{
	if (val >= 0 && val < (int)m_Options.size())
	{
		m_iSelectedIndex = val;
	}
}
